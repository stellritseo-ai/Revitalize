...
function RemodelingPage() {
  ...
  const [remodelType, setRemodelType] = useState("Kitchen Remodel");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let projectType = "remodeling";
      if (remodelType.toLowerCase().includes("kitchen")) projectType = "kitchen";
      else if (remodelType.toLowerCase().includes("bath")) projectType = "bathroom";
      ...
      await addLead({
        ...
        description: `Remodeling Request: Remodel Type: ${remodelType}, Preferred Meeting: ${meeting}, Financing Interest: ${interestFinancing ? "Yes" : "No"}, Material Info: ${interestMaterials ? "Yes" : "No"}`,
        contactTime: "anytime"
      });
      ...
      await addWebEmail({
        ...
        email,
        phone,
        service: `Remodeling - ${remodelType}`,
      ...
      try {
        await fetch(`https://formsubmit.co/ajax/revitalizerealestate@gmail.com`, {
          ...
          body: JSON.stringify({
            _subject: `New Remodeling Request: ${name} - ${remodelType}`,
            _captcha: "false",
            name,
            contact,
            service: remodelType,
            ...
            <form onSubmit={handleSubmit} className="bg-gray-50 p-8 md:p-12 rounded-[3rem] border border-gray-100 shadow-inner grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 md:col-span-2 text-left">
                ...
                >
                  ...
                  <option value="Whole Home Transformation">Whole Home Transformation</option>
                  <option value="Other Home Improvement">Other Home Improvement</option>
                </select>
              </div>
              <div className="space-y-2 md:col-span-2 text-left">
                <label className="text-xs font-black uppercase tracking-widest text-gray-400 ml-4">
              ...
              >
                {isSubmitting ? "Submitting Request..." : "Submit Remodeling Request"} <ArrowRight className="w-6 h-6" />
              </button>
            </form>
          </div>
        </div>
